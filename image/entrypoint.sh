...
if [ "$(id -u)" = 0 ]; then
  if [ -n "${CLAUDE_BOX_PROXY_IP:-}" ]; then
    port="${CLAUDE_BOX_PROXY_PORT:-8080}"
...
table inet claudebox {
	chain output {
		...
		ip daddr ${CLAUDE_BOX_PROXY_IP} tcp dport ${port} accept
	}
}
EOF
    then
      echo "[claude-box] FATAL: could not install egress firewall (missing NET_ADMIN?)." >&2
      exit 1
    fi
  fi
